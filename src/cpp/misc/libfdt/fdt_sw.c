...
int fdt_end_node(void *fdt)
{
	...
	en = reinterpret_cast<fdt32_t*>(fdt_grab_space_(fdt, FDT_TAGSIZE));
	if (! en)
		return -FDT_ERR_NOSPACE;
...
int fdt_finish(void *fdt)
{
	...
	/* Add terminator */
	end = reinterpret_cast<fdt32_t*>(fdt_grab_space_(fdt, sizeof(*end)));
	if (! end)
		return -FDT_ERR_NOSPACE;
	*end = cpu_to_fdt32(FDT_END);
	...
	while ((tag = fdt_next_tag(fdt, offset, &nextoffset)) != FDT_END) {
		if (tag == FDT_PROP) {
			struct fdt_property *prop =
				reinterpret_cast<struct fdt_property*>(fdt_offset_ptr_w_(fdt, offset));
			int nameoff;
