						...
						uint32_t delta)
{
	...
	fdt_for_each_property_offset(fixup_prop, fdto, fixup_node) {
		...
		fixup_val = reinterpret_cast<const fdt32_t*>(fdt_getprop_by_offset(fdto, fixup_prop,
						  &name, &fixup_len));
		if (!fixup_val)
			return fixup_len;
				     ...
				     int poffset, const char *label)
{
	...
	symbol_path = reinterpret_cast<const char*>(fdt_getprop(fdt, symbols_off, label,
				  &prop_len));
	if (!symbol_path)
		return prop_len;
				 ...
				 int property)
{
	...
	value = reinterpret_cast<const char*>(fdt_getprop_by_offset(fdto, property,
				      &label, &len));
	if (!value) {
	...
	fdt_for_each_property_offset(prop, fdto, ov_sym) {
		...
		if ((e - s) > len && (hls::memcmp(s, "/__overlay__/", len) == 0)) {
			...
			rel_path_len = e - rel_path - 1;
			   ...
			   && (hls::memcmp(s, "/__overlay__", len - 1) == 0)) {
