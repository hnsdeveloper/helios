...
static void fdt_packblocks_(const char *old, char *new_blk,
			    int mem_rsv_size,
			    int struct_size,
			    int strings_size)
{
...
int fdt_open_into(const void *fdt, void *buf, int bufsize)
{
	...
	fdt_packblocks_(reinterpret_cast<const char*>(fdt), tmp, mem_rsv_size, struct_size,
...
int fdt_pack(void *fdt)
{
	...
	mem_rsv_size = (fdt_num_mem_rsv(fdt)+1)
		* sizeof(struct fdt_reserve_entry);
	fdt_packblocks_(reinterpret_cast<const char*>(fdt), reinterpret_cast<char*>(fdt), mem_rsv_size, fdt_size_dt_struct(fdt),
			fdt_size_dt_strings(fdt));
