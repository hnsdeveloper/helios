...
static int overlay_fixup_one_phandle(void *fdt, void *fdto, int symbols_off, const char *path, uint32_t path_len,
                                     const char *name, uint32_t name_len, int poffset, uint32_t phandle)
{
...
static int overlay_fixup_phandle(void *fdt, void *fdto, int symbols_off, int property)
{
    ...
    do
    {
        ...
        ret = overlay_fixup_one_phandle(fdt, fdto, symbols_off, path, path_len, name, name_len, poffset, phandle);
        if (ret)
            return ret;
    } while (len > 0);
 ...
 *      Negative error code on failure
 */
static int overlay_update_node_conflicting_references(void *fdto, int tree_node, int fixup_node, uint32_t fdt_phandle,
                                                      uint32_t fdto_phandle)
{
    ...
    fdt_for_each_subnode(fixup_child, fdto, fixup_node)
    {
        ...
        ret = overlay_update_node_conflicting_references(fdto, tree_child, fixup_child, fdt_phandle, fdto_phandle);
        if (ret)
            return ret;
    }
